from utils.ai_irw_utils import standardize, sampled_sphere
import numpy as np

# taken from https://github.com/GuillaumeStaermanML/AIIRW
...
def AI_IRW(X_test, AI=True, robust=False, n_dirs=None, X=None, random_state=None):
    ...
    if X_test is None:
        ...
        proj.sort(axis=0)
        for k in range(n_dirs):
            depth[:, k] = np.searchsorted(
                a=proj[:, k], v=proj_test[:, k], side="left")
